...
class RATSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RATSNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRATSNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : RATSNode {
        var curr : RATSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
